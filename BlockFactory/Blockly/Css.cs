...
namespace Blockly
{
	public static class Css
	{
		public class Cursor
		{
			public static readonly Cursor OPEN = new Cursor("handopen");
			public static readonly Cursor CLOSED = new Cursor("handclosed");
			public static readonly Cursor DELETE = new Cursor("handdelete");
			private string v;

			public Cursor(string v)
			{
				this.v = v;
			}
		...
		/// </summary>
		private static Cursor currentCursor_;

		/// <summary>
		/// Large stylesheet added by Blockly.Css.inject.
		/// </summary>
		private static CSSStyleSheet styleSheet_;

		/// <summary>
		/// Path to media directory, with any trailing slash removed.
		/// </summary>
		private static string mediaPath_;
		...
		{
			if (Css.currentCursor_ == cursor) {
				return;
			}
			Css.currentCursor_ = cursor;
			var url = "url(" + Css.mediaPath_ + "/" + cursor + ".cur), auto";
			...
			Css.styleSheet_.DeleteRule(0);
			Css.styleSheet_.InsertRule(rule, 0);
			...
			foreach (HTMLElement toolbox in toolboxen) {
				if (cursor == Css.Cursor.DELETE) {
